...
public class MultiplayerGameScreen implements Screen,InputProcessor {
    ...
    Texture kingTexture =  new Texture(Gdx.files.internal("king.png"));
    Texture knightTexture =  new Texture(Gdx.files.internal("knight.png"));
    Texture bishopTexture = new Texture(Gdx.files.internal("bishop.png"));
    Texture greenBorderImage = new Texture(Gdx.files.internal("green-border.png"));
    Texture redBorderImage = new Texture(Gdx.files.internal("red-border.png"));
    Texture back_button = new Texture(Gdx.files.internal("back_button.png"));
    Texture end_button = new Texture(Gdx.files.internal("end_button.png"));
    ...
    public void configSocket() {
        if(!sideSelect) {
            socket.on("attackerMove", new Emitter.Listener() {
                @Override
                public void call(Object... args) {
                    ...
                    try {
                        ...
                        MultiplayerGameScreen.this.attackerMove();
                    } catch (Exception e) {
                        System.out.println(e);
                    }
                }
        ...
        if(sideSelect) {
            socket.on("defenderMove", new Emitter.Listener() {
                @Override
                public void call(Object... args) {
                    ...
                    try {
                        ...
                        for (Piece currPiece : Piece.allPieces) {
                            ...
                            if (currPiece != null && currPiece.getClass().toString().equals(movePiece) && currPiece.getIndex() == index) {
                                MultiplayerGameScreen.this.currSelectedPiece = currPiece;
                                Position newPos = new Position(movX, movY);
                                MultiplayerGameScreen.this.currSelectedPiece.changePiecePos(newPos);
                                MultiplayerGameScreen.this.currSelectedPiece.changePieceViewPos();
                                break;
                            }
                        }
                        MultiplayerGameScreen.this.defenderMove();
                    } catch (Exception e) {
        ...
        socket.on("gameOver", new Emitter.Listener() {
            ...
            public void call(Object... args) {
                ...
                try {
                    ...
                    MultiplayerGameScreen.this.game.setScreen(new GameOver(MultiplayerGameScreen.this.game, points));
                } catch (Exception e) {
                    System.out.println(e);
                }
            }
        });
        socket.on("opponentDisconnect", new Emitter.Listener() {
            @Override
            public void call(Object... args) {
                MultiplayerGameScreen.this.game.setScreen(new DisconnectScreen(MultiplayerGameScreen.this.game));
            }
        });
    }
    ...
    public void render(float delta) {
        ...
        game.batch.draw(end_button, 1075, 620, 100,100);
        game.batch.draw(back_button, 925, 620, 100,100);
...
//		camera.unproject(tp.set(screenX, screenY, 0));
    ...
    @Override public boolean touchDown (int screenX, int screenY, int pointer, int button) {
        ...
        if(mouseY==0 && mouseX==7){
            ...
            socket.emit("playerDisconnect",discon);
            Gdx.app.exit();
            dispose();
            System.exit(0);
        }
        if(mouseY==0 && mouseX==6){
            JSONObject discon = new JSONObject();
            socket.emit("playerDisconnect",discon);
            MainMenu newGame = new MainMenu(game);
